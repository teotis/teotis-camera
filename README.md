...
- **独立的模式插件**：产品模式收敛为拍照、打卡、人文、视频和文档；人像、夜景、超清增强和专业能力由模式策略组合，再由会话内核统一调度。
- **可解释的增强拍摄**：静态拍摄可按设备能力选择 CameraX 扩展或多帧处理路径，并通过能力门控、降级语义和诊断信息保持行为可追踪。
- **实时色彩管线**：Color Lab 使用 `ColorLabSpec`、`StyleColorPipeline` 和 `PreviewColorTransform` 将色彩选择、风格配置和预览叠加拆开，既能实时预览，也能把配置保存为可复用风格。
- **可组合媒体后处理**：水印、画幅、人像、文档裁切、算法处理等能力以媒体管线后处理器组合，预览提示和最终成片共享同一批设置与效果契约，确保水印模板落到保存照片。
...
| `feature:mode-checkin` | 打卡模式：区域感知静态拍摄，覆盖人像、人景、物景和超清增强场景 | ✅ |
...
- **手势控制**：点击对焦、双指缩合、滑动调节
- **增强拍摄**：CameraX 扩展、多帧融合和能力感知降级
- **自拍镜像**：前置拍照按设置执行镜像后处理
- **方向感知**：自适应横竖屏 UI 布局
- **驾驶舱面板**：专业参数调节界面
- **色彩实验室**：实时色彩风格预览
- **水印系统**：可配置模板、预览提示和成片后处理
- **开发控制台**：运行时诊断和日志导出
