...
- `2026-05-26` 该 UI 动效 V2 调度包已按本机 Claude Code `2.1.142` 更新启动方式：不再使用旧式 `claude --bg <prompt>` 口径，改为 `claude agents --cwd /Volumes/Extreme_SSD/project/open_camera --permission-mode default --effort high` 打开 Agent View 后按 `agent-view-prompts.md` 分阶段派发；当前 CLI help 只暴露 Agent View 管理入口，没有暴露创建 Agent View 任务单位的子命令，因此 helper 脚本只做预检并打开 Agent View，任务单位需要在 Agent View 内粘贴对应 package prompt 创建。
