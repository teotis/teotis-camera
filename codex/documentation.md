...
- `2026-05-26` 该 UI 动效 V2 调度包已按本机 Claude Code `2.1.142` 更新启动方式：不再使用旧式 `claude --bg <prompt>` 口径，改为 `claude agents --cwd /Volumes/Extreme_SSD/project/open_camera --permission-mode default --effort high` 打开 Agent View 后按 `agent-view-prompts.md` 分阶段派发；helper 脚本现在只做预检并打开 Agent View，不自动启动全部包。
