...
[ -f "$PLAN_DIR/packages/01-current-iq-gap-audit.md" ] || { echo "ERROR: package 01 not found"; exit 1; }
[ -f "$PLAN_DIR/packages/02-style-target-scorecard.md" ] || { echo "ERROR: package 02 not found"; exit 1; }
...
echo
echo "=== Launching Group 1 Humanistic IQ research agents ==="
echo "Only G1 is launched by script. Launch packages 03, 04, and 05 after dependencies pass."
...
launch_agent() {
  ...
  prompt="Read $PLAN_DIR/INDEX.md and $package_doc. Execute research package $package_id. Write evidence to $status_file when done. Do NOT edit INDEX.md, runtime code, tests, package docs, or other status files. Do NOT claim vivo/system-camera parity."
  ...
  if [ "$status" -ne 0 ]; then
    ...
    if [ "$CLAUDE_PERMISSION_MODE" = "auto" ] && grep -qi "requires opting in" <<<"$output"; then
      ...
      echo "  CLAUDE_PERMISSION_MODE=default rtk bash docs/plans/humanistic-image-quality-tuning-orchestration/launchers/dispatch-claude-agents.sh" >&2
    fi
    return "$status"
  fi
...
launch_agent \
  ...
  "$PLAN_DIR/packages/01-current-iq-gap-audit.md" \
  "$PLAN_DIR/status/01-current-iq-gap-audit.md" \
  "agent-01-iq-gap-audit"

launch_agent \
  "02-style-target-scorecard" \
  "$PLAN_DIR/packages/02-style-target-scorecard.md" \
  "$PLAN_DIR/status/02-style-target-scorecard.md" \
  "agent-02-style-scorecard"
...
echo "After package 04 completes, launch package 05."
