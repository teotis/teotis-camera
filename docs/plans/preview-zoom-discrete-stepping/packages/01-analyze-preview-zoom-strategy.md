...
1. **镜头检测分析**：分析 `detectLensNodeMap()` 在 参考设备 上的实际分类结果。当前启发式 `maxZoom >= 5f → PERISCOPE, >= 1.6f → TELEPHOTO` 可能将广角镜头（带高倍数字变焦）错误分类。如果存在问题，记录改进方案但不在此 package 实现。
...
- 当前 `detectLensNodeMap()` 只检测 WIDE/TELEPHOTO/PERISCOPE 三种镜头类型，不区分超广角和广角。如果 参考设备 有三颗后摄，可能存在分类不准确。
