...
对 `public/teotis-camera` 公开仓进行全面暴露检查，覆盖 Git 历史身份、工作区文件、图片元数据、README/NOTICE/AUTHORS、竞品引用等维度。
...
```
所有 5 个 commit 的 author 和 committer 均为 `dingren <dingren@xiaomi.com>`。

**根本原因**：公开仓 `git init` 后未配置本地 `user.name` / `user.email`，导致继承全局 `~/.gitconfig` 中的 `user.name=dingren` / `user.email=dingren@xiaomi.com`。
...
- 风险上：公开仓中出现竞品真机型号可能被解读为"参考竞品"或"从竞品逆向"
...
README.md 和 README_EN.md 中的架构描述均为通用技术文档（状态驱动 UI、模式插件、色彩管线等），不包含竞品比较或"学习参考"措辞。内容干净。
...
所有 5 个 commit message 均为中性描述（`feat:`, `fix:`, `docs:`, `chore:`），不包含竞品引用、个人身份或敏感信息。
...
| 工作区身份 | `grep -rn 'dingren\|dingren@\|/Users/\|丁仁'` | 无泄露（文件已清理） |
| 竞品引用 | `grep -rn 'Apple\|iPhone\|vivo\|Xiaomi\|MIUI\|MiuiCamera\|Leica\|Hasselblad'` | 3 处（test fixture） |
| 宽范围竞品 | `grep -rn 'xiaomi\|miui\|oppo\|huawei\|samsung\|pixel'` | 0 处 |
...
**当前公开仓存在 P0 级身份泄露**：所有 5 个 commit 的 author 和 committer 均为 `dingren <dingren@xiaomi.com>`，已推送到 GitHub 远端。任何人都可通过 `git log` 查看开发者真实姓名和公司邮箱。

工作区文件已清理干净（无个人路径、无竞品注释），但**Git 历史是公开面的一部分**，仅清理工作区不够。
