...
PLAN_DIR="$REPO_ROOT/docs/plans/real-device-preview-frame-watermark-polish-orchestration"
...
launch_agent() {
  ...
  local prompt
  prompt="Read $PLAN_DIR/INDEX.md and $package_doc. Implement package $package_id. Write evidence to $status_file when done. Do NOT edit INDEX.md or other status files. Run repo shell commands through rtk and use isolated Gradle in worktrees."
  echo "Launching $name"
  set +e
  ...
  if [ "$status" -ne 0 ]; then
    ...
    if [ "$CLAUDE_PERMISSION_MODE" = "auto" ] && grep -qi "requires opting in" <<<"$output"; then
      ...
      echo "Or rerun without auto mode: CLAUDE_PERMISSION_MODE=default rtk bash docs/plans/real-device-preview-frame-watermark-polish-orchestration/launchers/dispatch-claude-agents.sh" >&2
    fi
    return "$status"
  fi
  echo "$output"
...
launch_agent "01-preview-frame-contract" "$PLAN_DIR/packages/01-preview-frame-contract.md" "$PLAN_DIR/status/01-preview-frame-contract.md"
launch_agent "02-natural-blur-border-rendering" "$PLAN_DIR/packages/02-natural-blur-border-rendering.md" "$PLAN_DIR/status/02-natural-blur-border-rendering.md"
...
echo
echo "After both agents complete, run the integration audit."
