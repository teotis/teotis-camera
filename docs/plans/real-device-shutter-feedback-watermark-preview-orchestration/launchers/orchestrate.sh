...
MAX_PARALLEL="${ORCHESTRATION_MAX_PARALLEL:-3}"
...
validate_status_state() {
  local package_id="$1"
  local status_rel status_file md st
...
launch_package() {
  ...
  name="shutter-feedback-watermark-${package_id}"
  prompt="$(build_prompt "$package_id")"
  launched="$(now_utc)"
  echo "Launching $name"
  set +e
  if [ -n "$CLAUDE_PERMISSION_MODE" ]; then
    output="$(claude --bg --name "$name" --model "$CLAUDE_MODEL" --effort "$CLAUDE_EFFORT" --permission-mode "$CLAUDE_PERMISSION_MODE" --setting-sources "$CLAUDE_SETTING_SOURCES" "$prompt" </dev/null 2>&1)"
  else
    output="$(claude --bg --name "$name" --model "$CLAUDE_MODEL" --effort "$CLAUDE_EFFORT" --setting-sources "$CLAUDE_SETTING_SOURCES" "$prompt" </dev/null 2>&1)"
  fi
  status=$?
  set -e
  if [ "$status" -ne 0 ]; then
...
launch_finalize_if_ready() {
  local final_pkg st
  final_pkg="$(finalize_package)"
  st="$(state_field "$final_pkg" state)"
  if all_functional_completed && { [ "$st" = "pending" ] || [ "$st" = "ready" ]; }; then
    launch_package "$final_pkg"
  else
    echo "Finalize is not ready."
  fi
}

cmd_status() {
  preflight
  printf "%-42s %-12s %-52s %-16s %s\n" "package" "state" "branch" "verification" "last_error"
  awk -F '\t' 'FNR > 1 { printf "%-42s %-12s %-52s %-16s %s\n", $1, $2, $6, $10, $13 }' "$STATE"
}
...
cmd_retry() {
  ...
  case "$st" in
    blocked|stale|invalid)
      echo "Retrying $package_id; previous error: $(state_field "$package_id" last_error)"
      set_state_field "$package_id" state ready
      set_state_field "$package_id" last_error ""
      set_state_field "$package_id" launched_at ""
      set_state_field "$package_id" agent ""
      launch_ready
      ;;
    *)
      die "can only retry blocked, stale, or invalid packages; $package_id is $st"
      ;;
  esac
}
...
cmd_advance() {
  ...
  while [ "${1:-}" != "" ]; do
    case "$1" in
      --from)
        shift
        echo "Advance hint from ${1:-unknown}; recomputing readiness from state."
        ;;
    esac
    shift || true
  done
