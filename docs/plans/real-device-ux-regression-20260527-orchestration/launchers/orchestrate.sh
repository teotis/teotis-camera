...
preflight_graph() {
  ...
  awk -F '\t' '
    ...
    END {
      for (id in ids) {
        if (visit(id)) {
          printf("ERROR: dependency cycle includes %s\n", cycle_node) > "/dev/stderr"
          exit 1
        }
      }
    }
    function visit(id, arr, i, dep) {
      if (visiting[id]) {
        cycle_node = id
        return 1
      }
...
launch_package() {
  ...
  name="ux-regression-0527-${package_id}"
  now="$(timestamp)"
  ...
  prompt="You are executing Real Device UX Regression 20260527 package $package_id for OpenCamera.
