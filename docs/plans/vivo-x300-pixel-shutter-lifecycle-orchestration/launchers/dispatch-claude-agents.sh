...
launch_agent() {
  local package_id="$1"
  local depends_on="$2"
  local name="vivo-x300-${package_id}"
  ...
  prompt="You are executing OpenCamera research/design package $package_id.
...
When done, write the full evidence pack to $status_file: worktree path, branch, git status, git diff --stat, changed files, commands run, test results, source/code references, acceptance criteria status, recommended decision, unresolved risks, and self-certification that you only touched allowed paths."
  ...
  if [ "$status" -ne 0 ]; then
    ...
    if [ "$CLAUDE_PERMISSION_MODE" = "auto" ] && grep -qi "requires opting in" <<<"$output"; then
      ...
      echo "Or rerun this script without a permission override so user-level settings apply: unset CLAUDE_PERMISSION_MODE; rtk bash docs/plans/vivo-x300-pixel-shutter-lifecycle-orchestration/launchers/dispatch-claude-agents.sh" >&2
    fi
    return "$status"
  fi
  echo "$output"
...
echo "=== Vivo X300 Pixel/Shutter Research Dispatch ==="
echo "Plan: $PLAN_DIR"
echo "Phase: $PHASE"
...
case "$PHASE" in
  opt-in-auto)
    ...
    ;;
  g1)
    launch_agent "01-pixel-capability-enumeration" "none"
    launch_agent "03-shutter-lifecycle-contract" "none"
    ;;
  g2)
    launch_agent "02-quick-pixel-surface-design" "01-pixel-capability-enumeration completed"
    ;;
  g3)
    launch_agent "04-real-device-verification-protocol" "01, 02, and 03 completed"
    ;;
  audit|g4)
    launch_agent "99-integration-audit" "all research/design status files completed"
    ;;
  all)
    echo "WARNING: launching all phases can create dependency-waiting agents. Prefer g1 -> g2 -> g3 -> audit."
    launch_agent "01-pixel-capability-enumeration" "none"
    launch_agent "03-shutter-lifecycle-contract" "none"
    launch_agent "02-quick-pixel-surface-design" "01-pixel-capability-enumeration completed"
    launch_agent "04-real-device-verification-protocol" "01, 02, and 03 completed"
    ;;
  --print-only)
    echo "No agents launched. Use one of: opt-in-auto, g1, g2, g3, audit, all."
    exit 0
    ;;
  *)
    echo "ERROR: unknown phase '$PHASE'"
    echo "Use one of: opt-in-auto, g1, g2, g3, audit, all, --print-only"
    exit 1
    ;;
esac
