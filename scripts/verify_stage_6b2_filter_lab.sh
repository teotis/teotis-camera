...
volatile_paths=(
  ...
  "${shared_build_root}/app/intermediates/incremental/debug/mergeDebugResources"
  "${shared_build_root}/app/kotlin"
  "${shared_build_root}/app/snapshot"
  "${shared_build_root}/core/settings/kotlin"
  "${shared_build_root}/core/settings/snapshot"
  ...
  "${shared_build_root}/feature/mode-photo/kotlin"
  "${shared_build_root}/feature/mode-photo/snapshot"
  "${shared_build_root}/feature/mode-humanistic/kotlin"
  "${shared_build_root}/feature/mode-humanistic/snapshot"
  "${shared_build_root}/feature/mode-portrait/kotlin"
  "${shared_build_root}/feature/mode-portrait/snapshot"
  "${shared_build_root}/feature/mode-video/kotlin"
  "${shared_build_root}/feature/mode-video/snapshot"
)
...
reset_volatile_state() {
  ...
  for hashed_root in "${shared_build_root}"/*; do
    ...
    for candidate in \
      ...
      "${hashed_root}/app/intermediates/incremental/debug/mergeDebugResources" \
      "${hashed_root}/app/kotlin" \
      "${hashed_root}/app/snapshot" \
      "${hashed_root}/core/settings/kotlin" \
      "${hashed_root}/core/settings/snapshot" \
      ...
      "${hashed_root}/feature/mode-photo/kotlin" \
      "${hashed_root}/feature/mode-photo/snapshot" \
      "${hashed_root}/feature/mode-humanistic/kotlin" \
      "${hashed_root}/feature/mode-humanistic/snapshot" \
      "${hashed_root}/feature/mode-portrait/kotlin" \
      "${hashed_root}/feature/mode-portrait/snapshot" \
      "${hashed_root}/feature/mode-video/kotlin" \
      "${hashed_root}/feature/mode-video/snapshot"; do
      if [[ -e "${candidate}" ]]; then
        rm -rf "${candidate}"
        printf 'Removed volatile build state: %s\n' "${candidate}"
        removed_count=$((removed_count + 1))
  ...
  if [[ "${removed_count}" -eq 0 ]]; then
    printf 'No volatile stage-6b2 build state found under %s\n' "${shared_build_root}"
  fi
}
...
assert_no_appledouble_noise() {
  ...
  while IFS= read -r path; do
    ...
    cd "${project_root}" &&
      find feature/mode-photo feature/mode-humanistic feature/mode-portrait feature/mode-video scripts \
        -name '._*' | sort
  )
...
remove_appledouble_noise() {
  ...
  if [[ "${removed_any}" -eq 0 ]]; then
    printf 'No AppleDouble noise found under filter-related feature modules or scripts\n'
  fi
}
...
run_gradle :core:settings:test \
  --tests com.opencamera.core.settings.PersistedSettingsSerializerTest \
  --tests com.opencamera.core.settings.FilterProfileShareCodecTest
...
run_gradle :app:testDebugUnitTest \
  ...
  --tests com.opencamera.app.camera.PhotoAlgorithmPostProcessorTest
